...
def main(document_list, stopwordfile):
    ...
    for doc in document_tokens:
        vectors.append(vectorise(doc, inverse_index, idf))
    X = normalize(np.array(vectors), axis=1)
    print(X.shape)
    for x in X:
        print()
    np.savetxt("dv.csv",X,delimiter=",")
    clusters = cluster.KMeans(n_clusters=2, random_state=123).fit_predict(X)
    print(clusters)
    clusters = cluster.DBSCAN().fit_predict(X)
    print(clusters)
    return None
